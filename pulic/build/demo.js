        ...
        if (typeof delta === 'undefined') {
            ...
            switch (selectedType) {
                case 'visual':
                    ...
                    if (!document.getElementById('showunchanged')
                        .checked) {
...
const getSelectedDeltaType = function () {
    if (document.getElementById('show-delta-type-visual')
        .checked) {
        return 'visual';
    }
    if (document.getElementById('show-delta-type-annotated')
        .checked) {
        return 'annotated';
    }
    if (document.getElementById('show-delta-type-json')
        .checked) {
        return 'json';
    }
};
...
document
    ...
    htmlFormatter.showUnchanged(document.getElementById('showunchanged').checked, null, 800);
});
...
const load = {
    data: function (dataArg) {
        ...
        const leftValue = data.left
            ? data.left.content || data.left
            : '';
        areas.left.setValue(leftValue);
        const rightValue = data.right
            ? data.right.content || data.right
            : '';
